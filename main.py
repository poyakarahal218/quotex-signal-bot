...
# توكن البوت الخاص بك مدمج ومنضبط تماماً
...
# قائمة شاملة لجميع أزواج العملات العادية والـ OTC
PAIRS_MAP = {
    "EUR/USD": "EURUSD",
    "GBP/USD": "GBPUSD",
    "USD/JPY": "USDJPY",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اختيار مدة الصفقة ذكياً بناءً على خصائص الزوج (مثلاً أزواج OTC والتقلبات العالية تأخذ 5 دقائق، والأزواج العادية القوية 4 دقائق)
    if "OTC" in pair:
        calculated_duration = 5  # أزواج الـ OTC تحتاج غالباً وقتاً أكبر لتصحيح السيولة
        analysis_reason = "تذبذب OTC مرتفع (اختيار 5 دقائق لاستيعاب إعادة اختبار السيولة)"
    else:
        calculated_duration = 4  # أزواج السوق العادي بناءً على زاوية الزخم
        analysis_reason = "قوة زخم هيكلي BOS (اختيار 4 دقائق لتوافق حركة الشموع)"

    # رسالة تحليلية متقدمة ومنسقة بدقة فائقة
    analysis_text = (
...
def main():
    ...
    logger.info("Bot is starting with smart technical duration selection...")
